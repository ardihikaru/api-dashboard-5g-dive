...
from .cpu_core_functions import get_all_data, del_all_data, get_latest_data, insert_new_data
...
class CpuCore(CpuCoreModel):
    ...
    def trx_register(self, ses, json_data):
        ...
        if is_valid:
            ...
            _, json_data = insert_new_data(ses, CpuCoreModel, json_data)
            self.set_msg(msg)
    ...
    def trx_get_latest(self, ses):
        is_valid, db_data, msg = get_latest_data(ses, CpuCore)
        if db_data is None:
            is_valid = False
            msg = "CPU Core data not found"
        self.set_resp_status(is_valid)
