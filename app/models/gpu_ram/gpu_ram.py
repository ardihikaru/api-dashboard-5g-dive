...
from .gpu_ram_model import GpuRamModel
from .gpu_ram_functions import get_all_data, del_all_data, get_latest_data, insert_new_data
import simplejson as json


class GpuRam(GpuRamModel):
    def __init__(self):
    ...
    def trx_register(self, ses, json_data):
        ...
        if is_valid:
            msg = "Adding new log data `gpu RAM utilization` succeed."
            _, json_data = insert_new_data(ses, GpuRamModel, json_data)
            self.set_msg(msg)
    ...
    def trx_get_data(self, ses, get_args=None):
        is_valid, data = get_all_data(ses, GpuRam)
        self.set_resp_status(is_valid)
    ...
    def trx_del_all_data(self, ses):
        is_valid, db_data, msg = del_all_data(ses, GpuRam)
        if db_data is None:
            is_valid = False
            msg = "gpu RAM data not found"
        self.set_resp_status(is_valid)
        self.set_msg(msg)
        if is_valid:
    ...
    def trx_get_latest(self, ses):
        is_valid, db_data, msg = get_latest_data(ses, GpuRam)
        if db_data is None:
            is_valid = False
            msg = "gpu RAM data not found"
        self.set_resp_status(is_valid)
